...
Never commit plaintext credentials; instead edit `secrets/*.age` via `nix shell .#ragenix -c ragenix -e secrets/<name>.age` so recipients from `secrets/secrets.nix` remain authoritative (use `nix shell`, not `nix run` — ragenix needs to be on `PATH` for the edit flow). To write a secret non-interactively (agents, scripts, piping in a token), point `$EDITOR` at a command that overwrites ragenix' temp file with the plaintext — ragenix invokes `$EDITOR <tempfile>`, so a fake editor like `EDITOR='cp token.txt' nix shell .#ragenix -c ragenix -e secrets/<name>.age` seeds `<tempfile>` from `token.txt` (use `printf %s` to write `token.txt` without a trailing newline). Creating a *new* secret needs only the recipient pubkeys; editing an *existing* one also needs a matching recipient private key on the box to decrypt it first. Collect new host keys with `ssh-keyscan -t ed25519 <host>` before adding them to `secrets/secrets.nix` or ragenix’ recipients; this keeps age policies in sync with reality. New services must declare their secret inputs in the target host’s module (and document ownership inline), and any shared Tailscale/Restic keys should be rotated before expiry. Treat `.env` only for short-lived experiments—persistent values belong in agenix and must be referenced through the module system.
