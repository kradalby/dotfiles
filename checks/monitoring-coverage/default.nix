...
# up or explicitly allowlisted below with a reason — that decision is the point.
{
  ...
  monitoredHosts =
    if nodesJob == null
    then []
    else map (t: lib.head (lib.splitString ":" t)) (targetsOf nodesJob);
  ...
  exporterGaps =
    lib.concatMap (
      host:
        lib.concatMap (
          e:
            if (exemptExporters ? ${e.name}) || e.port == null || isMonitored host e.port
            then []
            else ["${host}: prometheus.exporters.${e.name} (:${toString e.port}) is enabled but no scrape job targets it"]
  ...
  hostGaps =
    lib.concatMap (
      host:
        if (exemptHosts ? ${host}) || lib.elem host monitoredHosts
        then []
        else ["${host}: NixOS host has no node_exporter scrape (add it to allHosts)"]
    )
    hostNames;
...
in
  pkgs.runCommand "monitoring-coverage" {} (
    if gaps == []
    then "echo 'monitoring-coverage: every enabled exporter is scraped and every host is onboarded' >&2; touch $out"
    else ''
