...
let
  prodRoute = self.nixosConfigurations.core-oracldn.config.services.prometheus.alertmanager.configuration.route;
  fastRoute =
    prodRoute
    // {
      group_wait = "1s";
      group_interval = "2s";
      repeat_interval = "5s";
      routes = map (r: r // {group_wait = "1s"; repeat_interval = "5s";}) prodRoute.routes;
    };
in
