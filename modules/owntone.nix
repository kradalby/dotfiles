...
let
  ...
  formatValue =
    ...
    if isBool v then
      ...
      "{ ${concatMapStringsSep ", " (x: ''"${x}"'') v} }"
    else
      throw "owntone: unsupported config value type";
  ...
  toLibConfuse =
    ...
    concatStringsSep "\n\n" (
      mapAttrsToList (
        section: values:
        "${section} {\n${
          concatStringsSep "\n" (mapAttrsToList (key: value: "\t${key} = ${formatValue value}") values)
        }\n}"
      ) settings
    );
...
in
{
  ...
  config = mkMerge [
    ...
    (mkIf (cfg.enable && ccfg.enable) {
      ...
      systemd.services.p3-controller =
        let
