...
let
  ...
  mkArgs =
    name: ic:
    let
      instanceName = if ic.name == "" then name else ic.name;
      instancePrefix = if ic.prefix == "" then name else ic.prefix;
    in
    [
    ...
    ++ lib.optionals (ic.spawn != "session") [
      "--capacity"
      (toString ic.capacity)
    ]
    ++ lib.optional ic.verbose "--verbose"
    ++ lib.optional (!ic.createSessionInDir) "--no-create-session-in-dir"
    ++ [
      (if ic.sandbox then "--sandbox" else "--no-sandbox")
    ];
  ...
  normalizeRemotes = lib.concatMapStringsSep "\n" (
    ic:
    let
