...
let
  ...
  curlAddressArgs =
    guiAddress: path:
    if isUnixGui guiAddress then
      "--unix-socket ${guiAddress} http://.${path}"
    else
      "${guiAddress}${path}";
            ...
            devices =
              ...
              map (
                ...
                if builtins.isString device then
                  { deviceId = icfg.settings.devices.${device}.id; }
                else if builtins.isAttrs device then
                  { deviceId = icfg.settings.devices.${device.name}.id; } // device
                else
                  throw "Invalid type for devices in folder; expected string or attrset."
              ) folderDevices;
          }
                  ...
                  { name, ... }: {
                    ...
                    options = {
                      ...
                      path = lib.mkOption {
                        type = lib.types.str // {
                          check =
                            x: lib.types.str.check x && (builtins.substring 0 1 x == "/" || builtins.substring 0 2 x == "~/");
...
in
{
  ...
  config = lib.mkIf (cfg != { }) {
    ...
    users.groups.syncthing = lib.mkIf (lib.any (icfg: icfg.enable && icfg.group == "syncthing") (
      lib.attrValues cfg
    )) { };

    environment.systemPackages = lib.concatMap (icfg: lib.optional icfg.enable icfg.package) (
      lib.attrValues cfg
    );
                ...
                let
                  args = lib.escapeShellArgs (
                    (lib.cli.toCommandLineGNU { } {
                      "no-browser" = true;
                      "gui-address" = (if isUnixGuiAddr then "unix://" else "") + icfg.guiAddress;
