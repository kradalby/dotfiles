...
in
  with builtins; {
    ...
    # rclone Jottacloud — single-use personal login token, one per host, never
    # shared (sharing invalidates the token family). See modules/rclone-jotta.nix.
    ...
    # WIFI
    "ldn-wifi.age".publicKeys =
      u
      ++ [
        hosts.dev-ldn
        hosts.rpi5-ldn
      ];
    "kphone15-wifi.age".publicKeys =
      u
      ++ [
        hosts.core-ldn
        hosts.dev-ldn
