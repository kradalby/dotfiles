...
The `garnix-ci` flake input tracks **`github:znaniye/garnix-ci/selfhost`** directly
— Samuel (znaniye) is the active upstream-of-record. (Repo is `garnix-ci`; bare
`garnix` 404s.) Pull his fixes with `nix flake update garnix-ci`. (No kradalby fork
branch is needed for the input; the local `integration`/`selfhost` branches at
`~/git/garnix` are just a working mirror if you ever want to carry local patches.)
...
## Create the Incus VM (declarative — `~/git/infrastructure/incus`)

VMs are infrastructure-as-code, NOT hand-rolled. Declare the VM in
`~/git/infrastructure/incus/gigabuilder_instances.tf` (mirror `ldn_instances.tf`'s
`incus_instance.ldn_ts1p`) with `remote = "gigabuilder"`, then `tofu apply` from
the **laptop** (which has the `gigabuilder` incus remote + the secrets via direnv).

VM (not container) — containers can't safely provide `/dev/kvm`. Key config:
`type = "virtual-machine"`, `image = "images:nixos/unstable"`, 8 cpu / 16 GiB,
`security.secureboot=false`, `linux.kernel_modules = "kvm_amd"`,
`raw.qemu = "-cpu host"` (nested virt for the action-runner); nic `bridged`
`parent = "incusbr0"` with `ipv4.address = "10.68.10.10"`; root disk
`pool = "default"` (gigabuilder's `vmpool`-backed ZFS pool — NOT `local`),
`size = "80GiB"`; a `file` block pushing the bootstrap root authorized_keys.

```bash
cd ~/git/infrastructure/incus
direnv exec . tofu plan      # run from the laptop (has the gigabuilder remote)
direnv exec . tofu apply
```

The VM boots `images:nixos/unstable`, then colmena applies `machines/garnix`
(disk layout from `common/incus.nix`: `/dev/sda1` vfat ESP + `/dev/sda2` ext4
root). Verify nested virt: `incus exec gigabuilder:garnix -- ls -l /dev/kvm`
(host nested virt confirmed: `kvm_amd nested=1`).
